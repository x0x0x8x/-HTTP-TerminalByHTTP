...
gl_handle_type = 1 #0: normal; 1: http
if gl_handle_type == 1:
    import HTTPSClient
    ...
    try:
        while inputm.getKeepState():
            ...
            if key == '\t':  # Tab
                ...
                backspace_handle(input_buffer, backCnt)
                pass
            elif key == '\x1b[A' or key == '\x1bOA':
                #print('UP')
                if historyIndex < len(__gl_historyInputBuffer) and historyIndex >= 0:
                ...
                #print(f'unkown key: {repr(key)}({len(key)})')
...
def http_login(ip = '127.0.0.1', port = 8000):
    url = f'http://{ip}:{port}/PhotoGallery/UploadImg'
